...
app.post('/deviceAdmin', function(req, res) {
  ...
  connection.getConnection(function(err,connection_callback){
    if(err){
      ...
      connection_callback.query("select a.device_id,b.device_id,b.customer_name,b.address,b.coordinates,b.gsm_mobile_number,b.key_location,b.device_password,b.config_password,c.power_level,c.gas_leak,c.low_gas,c.log_time from user_device_list a inner join devicelist b ON a.device_id=b.device_id left join device_log_current c on a.device_id=c.device_id where a.user_id='"+data+"'", function (err, result, fields) {
      res.send(result);
    });
connection_callback.end();  
});    
