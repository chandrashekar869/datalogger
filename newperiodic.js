...
function get_state_updated(res,exec){   
    ...
    connection.connect(function(err){
       ...
       var device_id_query="Select device_id from devicelist where session_id='"+message.split("&&")[0]+"'";
    ...
    connection.connect(function(err){
        ...
        connection.query(device_id_query,function(err,result,fields){
            ...
            if(result.length==0){
                ...
                res.send(login_tokenised_message.dummy_session_id+"&&"+login_tokenised_message.sfd+"&&"+login_tokenised_message.msgid+"&&"+1+"&&"+returnreq_id+"&&"+login_tokenised_message.Trans_Id);                    
            }
            else{
                    var sessid_new=Math.floor(Math.random()*89999999+10000000);
                    update_user_device_list(sessid_new,login_tokenised_message.devid);
                    res.send(sessid_new+"&&"+login_tokenised_message.sfd+"&&"+login_tokenised_message.msgid+"&&"+0+"&&"+returnreq_id+"&&"+login_tokenised_message.Trans_Id+"</h3>");
                }
        });
    }); 
}
...
function getDeviceId(){
 ...
 connection.connect(function(err){
    if(err) throw err;
    console.log("Connected");
    var device_id_query="Select device_id from devicelist where session_id='"+tokenisedobj.sessid+"'";
    console.log(device_id_query);
    connection.query(device_id_query,function(err,result,fields){
        if(result.length==0)
            return 1;
