...
app.post('/device/gaugesInfo', function(req, res){
  ...
  connection_callback.query("select distinct b.device_password, a.device_Id,a.tank_pressure,a.line_pressure,a.gas_level,a.gas_detector,a.gas_leak,a.low_gas,a.power_level,a.log_time,a.meter1,a.meter2,a.meter3,a.meter4,a.customer_name,a.solenoid log_solenoid, control_data.solenoid control_solenoid ,control_data.device_state_updated from user_device_list c,   device_log_current a inner join devicelist b on a.device_Id=b.device_id  LEFT JOIN control_data ON a.device_Id = control_data.device_id where a.device_Id='"+device_id+"' and  c.device_id=a.device_Id", function (err, result, fields){
    if (err) throw err;
    res.send(result);
   }); 
  console.log("select b.device_password, a.device_Id,a.tank_pressure,a.line_pressure,a.gas_level,a.gas_detector,a.gas_leak,a.low_gas,a.power_level,a.log_time,a.meter1,a.meter2,a.meter3,a.meter4,a.customer_name,a.solenoid log_solenoid, control_data.solenoid control_solenoid ,control_data.device_state_updated from  device_log_current a inner join user_device_list b on a.device_Id=b.device_id  LEFT JOIN control_data ON a.device_Id = control_data.device_id where a.device_Id='"+device_id+"' and b.user_id='1234'");
...
app.post('/users/deviceList', function(req, res) {
 ...
 connection_callback.query("SELECT a.device_id ,b.gas_leak,b.low_gas,b.power_level,c.coordinates,log_time FROM user_device_list a,device_log_current b,devicelist c where  a.user_id='"+user_id+"' and a.device_id = b.device_Id and a.device_id = c.device_id", function (err, result, fields) {
...
app.post('/users/login', function(req, res){
  ...
  var password = req.body.password;
  connection_callback.query("SELECT * FROM user_details where user_id ='"+username+"' and password = '"+password+"'", function (err, result, fields){
  if (err){throw err;}
  if(result.length>0){
      res.send(result);
