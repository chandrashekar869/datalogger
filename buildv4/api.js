...
app.post('/thirtydaydata', function(req, res){
    ...
    if(queryid=="true")
        query="SELECT log_time,gas_level FROM device_log_historical USE INDEX (idx_report) WHERE device_id='"+device_Id+"' AND   log_time >= '"+enddate.getFullYear()+"-"+(enddate.getMonth()+1)+"-"+enddate.getDate()+"' order by log_time";
    if(queryid=="false")
        query="SELECT log_time,gas_detector FROM device_log_historical USE INDEX (idx_report)  WHERE device_id='"+device_Id+"' AND   log_time >= '"+enddate.getFullYear()+"-"+(enddate.getMonth()+1)+"-"+enddate.getDate()+"' order by log_time";
